document.addEventListener('DOMContentLoaded', async function () {
    ...
    const conditionMapping = {
        ...
        '#RSE': null, // Handled separately
        '#ESU': null // Handled separately
    };
        ...
        try {
            ...
            filteredRecords = records.filter(record => {
                ...
                return strCodeArray.some(code => {
                    if (code === '#ERS' || code === '#RSE' || code === '#ESU') return true; // Handle separately
                    const condition = conditionMapping[code];
                    if (!condition) return false;
                    if (typeof condition === 'function') return condition(htmlContent);
                    if (Array.isArray(condition)) return condition.includes(entryTime.dayOfWeek);
                    return false;
                });
            });
            ...
            await handleESULogic(filteredRecords, entryTime);
    ...
    async function handleERSLogic(records, entryTime) {
        ...
        for (const rec of ersRecords) {
            ...
            for (const [date, time] of Object.entries(ersData.times[zmanType] || {})) {
            ...
            if (!earliestTime) {
                console.warn(`No ${zmanType} found in the Sunday to Friday range for ERS`);
                continue;
            }
    
            const formula = fields.Time_for_formula;
            if (formula) {
                const adjustedTime = convertToAmPm(applyTimeFormula(earliestTime, formula));
    ...
    async function handleRSELogic(records, entryTime) {
        const rseRecords = records.filter(rec => {
            const codes = rec.fields.strCode;
            const codeArray = Array.isArray(codes) ? codes : JSON.parse(codes.replace(/'/g, '"'));
            return codeArray.includes('#RSE');
        });
    
        if (rseRecords.length === 0) {
            console.warn("No #RSE records found for processing");
            return;
        }
        ...
        const rseUrl = `https://www.hebcal.com/zmanim?cfg=json&geonameid=5100280&start=${lastSundayStr}&end=${fridayStr}`;
        ...
        for (const rec of rseRecords) {
            ...
            let earliestTime = null;
            for (const [date, time] of Object.entries(rseData.times[zmanType] || {})) {
                const currentTime = new Date(time);
                if (!earliestTime || currentTime < earliestTime) {
                    earliestTime = currentTime;
                }
            ...
            if (!earliestTime) {
                console.warn(`No ${zmanType} found in the Sunday to Friday range for RSE`);
                continue;
            }
    
            const roundedTime = roundToNearestFiveMinutes(earliestTime);
            fields.Time = convertToAmPm(roundedTime.toTimeString().slice(0, 5)); // Update time in HH:mm AM/PM format
        }
    }
    
    // #ESU Logic
    async function handleESULogic(records, entryTime) {
        const esuRecords = records.filter(rec => {
            const codes = rec.fields.strCode;
            const codeArray = Array.isArray(codes) ? codes : JSON.parse(codes.replace(/'/g, '"'));
            return codeArray.includes('#ESU');
        });
    
        if (esuRecords.length === 0) {
            console.warn("No #ESU records found for processing");
            return;
        }
        ...
        const esuUrl = `https://www.hebcal.com/zmanim?cfg=json&geonameid=5100280&start=${lastSundayStr}&end=${fridayStr}`;
        const esuResp = await fetch(esuUrl);
        if (!esuResp.ok) {
            console.error('Failed to fetch ESU Zmanim data');
            return;
        }
        const esuData = await esuResp.json();
    
        for (const rec of esuRecords) {
            const fields = rec.fields;
            const zmanType = fields.strZman_Start_Time || "sunset";
    
            let earliestTime = null;
            for (const [date, time] of Object.entries(esuData.times[zmanType] || {})) {
                const currentTime = new Date(time);
                if (!earliestTime || currentTime < earliestTime) {
                    earliestTime = currentTime;
                }
            ...
            if (!earliestTime) {
                console.warn(`No ${zmanType} found in the Sunday to Friday range for ESU`);
                continue;
            }
    
            fields.Time = convertToAmPm(earliestTime.toTimeString().slice(0, 5)); // Update time in HH:mm AM/PM format
        }
    }
